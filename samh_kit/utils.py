...
class mystat():
    ...
    def corr(obs, pred):
        return (np.corrcoef(ma.masked_invalid(pred),
                            ma.masked_invalid(obs))[0, 1])
    ...
    def evaluate_model(obs, pred):
        ...
        metrics['R'] = "{:.2f}".format(
            ma.corrcoef(ma.masked_invalid(pred),
                        ma.masked_invalid(obs))[0, 1])
    ...
    def timezone_to_local(data, dim='time', source_tz='UTC', target_tz='Asia/Shanghai', replace=True):
        time_sh = pd.to_datetime(data[dim]).tz_localize(
            source_tz).tz_convert(target_tz).tz_localize(None)
        if replace is True:
